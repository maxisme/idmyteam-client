...
def allowed():
    ...
    valid_members = {}
    for key in members:
        if config.SOCKET_STATUS == config.SOCKET_CONNECTED or len(members[key]) >= config.MIN_TRAINING_IMAGES_PER_MEMBER:
    ...
    if config.SOCKET_STATUS == config.SOCKET_NOT_TRAINED:
        ...
        if len(valid_members) < 2:
            return """You must have more than {} training images for each member and a minimum of 2 members 
                to start your teams recognition model!""".format(config.MIN_TRAINING_IMAGES_PER_MEMBER), False, []
    elif config.SOCKET_STATUS == config.SOCKET_CLOSED:
        return "You are not connected to the ID My Team Socket. Please try again later!", True, []
...
def upload(conn):
    ...
    for m in members:
        ...
        for image in members[m]:
            zip.write(image, image.replace(config.CLASSIFIED_PATH, '/'), zipfile.ZIP_DEFLATED)
    zip.close()

    # upload zip of classsified images to idmy.team
    r = requests.post('https://idmy.team/upload', files={
        'ZIP': file_buff.getvalue()
    }, data={
