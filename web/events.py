...
class MemberDeleteHandler(view.BaseHandler):
    ...
    def get(self, member_id):
        ...
        if not functions.Member.remove(self.db_connect(), member_id, config.CLASSIFIED_PATH):
...
class MemberPermHandler(view.BaseHandler):
    ...
    def post(self, member_id):
        ...
        if not functions.Member.set_permission(self.db_connect(), member_id, permission):
...
class ClassifyHandler(view.BaseHandler):
    ...
    def post(self):
        for img_path in self.request.arguments['image_paths[]']:
            ...
            if functions.path_in_dir(img_path, config.UNCLASSIFIED_PATH) and os.path.isfile(img_path):
                os.remove(img_path)
